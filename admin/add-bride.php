...
<body>
    ...
    <center>
        <div class="box container">
            <br>
            <h3 style="color: #fdce64;">Add Bride Items</h3>
            <?php
                error_reporting(1);
                ...
                if(isset($_POST['add'])) {
                    ...
                    $query = mysqli_query($con, "INSERT INTO bride(dress, perfume, price, size, image) value ('$add_name', '$add_perfume', '$add_price', '$add_size', '$img')");

                    if($query) {
                        move_uploaded_file($_FILES['image']['tmp_name'],"img/".$img);
                        echo "<script>alert('New item has been successfully added.');</script>";
                        echo "<script>window.location.href = 'bride.php'</script>";
                    }
                    else {
