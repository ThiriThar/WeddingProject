...
<body>
    ...
    <center>
        <div class="box container">
            <br>
            <h3 style="color: #fdce64;">Add Bouquet Items</h3>
            <?php
                error_reporting(1);
                ...
                if(isset($_POST['add'])) {
                    ...
                    $query = mysqli_query($con, "INSERT INTO bouquet(name, price, image) value ('$add_name', '$add_price', '$img')");

                    if($query) {
                        move_uploaded_file($_FILES['image']['tmp_name'],"img/".$img);
                        echo "<script>alert('New item has been successfully added.');</script>";
                        echo "<script>window.location.href = 'bouquet.php'</script>";
                    }
                    else {
